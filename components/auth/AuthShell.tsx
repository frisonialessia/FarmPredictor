    ...
    <div className="min-h-screen grid lg:grid-cols-2">
      ...
      <div className="relative hidden lg:flex flex-col justify-between p-12 overflow-hidden">
        ...
        <div className="relative">
          <h2 className="text-white font-extrabold tracking-tight leading-[1.1]" style={{ fontSize: "clamp(1.8rem,2.6vw,2.6rem)" }}>Harvest when you<br />actually <span style={{ color: "var(--lime)" }}>can</span>.</h2>
          <ul className="mt-6 space-y-2.5">
            {["Plan and simulator on one engine", "The gap to optimal, measured in dollars", "Live weather, no jargon"].map((t) => (
              <li key={t} className="flex items-center gap-2.5 text-sm" style={{ color: "rgba(255,255,255,.82)" }}>
                <span className="grid place-items-center h-5 w-5 rounded-full shrink-0" style={{ background: "rgba(133,223,66,.2)", color: "var(--lime)" }}>✓</span>{t}
              </li>
            ))}
          </ul>
        </div>
        <p className="relative text-xs" style={{ color: "rgba(255,255,255,.5)" }}>Prototype · demo data</p>
      </div>

      {/* form panel */}
      <div className="flex flex-col justify-center px-6 py-12 sm:px-12 lg:px-16" style={{ background: "var(--bg)" }}>
        <div className="w-full max-w-sm mx-auto">
          <Link href="/" className="lg:hidden flex items-center gap-2.5 mb-8"><BrandMark size={28} /><span className="font-bold tracking-tight text-lg">FarmPredictor</span></Link>
