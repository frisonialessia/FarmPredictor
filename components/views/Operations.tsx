    ...
    <div className="fade-in">
      <div className="grid lg:grid-cols-3 gap-5 mb-5">
        ...
        <div className="card p-6"><h4 className="text-[15px] font-bold mb-1">{tr("Supplies")}</h4><p className="text-xs mb-4 text-muted">{tr("Stock vs. projected need")}</p>
          <div className="space-y-4">{SUP.map(([n, have, need], i) => { const pct = Math.min(100, ((have as number) / (need as number)) * 100); const sh = have < need; return (<div key={i}><div className="flex items-center justify-between mb-1"><span className="text-sm font-medium">{tr(n as string)}</span><span className="mono text-xs" style={{ color: sh ? "var(--warn)" : "var(--muted)" }}>{have}/{need}</span></div><div className="h-2 rounded-full" style={{ background: "var(--mint)" }}><div className="h-full rounded-full" style={{ width: `${pct}%`, background: sh ? "var(--warn)" : "var(--green)" }} /></div></div>); })}</div>
        </div>
      </div>
      <div className="grid lg:grid-cols-3 gap-5">
        <div className="card p-6"><h4 className="text-[15px] font-bold mb-1">{tr("Fuel & diesel")}</h4><p className="text-xs mb-4 text-muted">{tr("Tank levels and weekly burn")}</p>
          <div className="space-y-4">{FUEL.map(([n, v], i) => { const low = (v as number) < 40; return (<div key={i}><div className="flex items-center justify-between mb-1"><span className="text-sm font-medium flex items-center gap-1.5"><Icon name="fuel" size={14} />{tr(n as string)}</span><span className="mono text-xs" style={{ color: low ? "var(--warn)" : "var(--muted)" }}>{v}%</span></div><div className="h-2 rounded-full" style={{ background: "var(--mint)" }}><div className="h-full rounded-full" style={{ width: `${v}%`, background: low ? "var(--warn)" : "var(--green)" }} /></div></div>); })}<div className="flex items-center justify-between pt-1"><span className="text-sm">{tr("Weekly burn")}</span><span className="mono text-sm font-bold">620 gal</span></div></div>
        </div>
        <div className="card p-6"><h4 className="text-[15px] font-bold mb-1">{tr("Irrigation")}</h4><p className="text-xs mb-4 text-muted">{tr("Soil moisture by parcel")}</p>
          <div className="space-y-4">{IRR.map(([n, v], i) => { const dry = (v as number) < 45; return (<div key={i}><div className="flex items-center justify-between mb-1"><span className="text-sm font-medium flex items-center gap-1.5"><Icon name="drop" size={14} />{n}</span><span className="mono text-xs" style={{ color: dry ? "var(--warn)" : "var(--muted)" }}>{v}%</span></div><div className="h-2 rounded-full" style={{ background: "var(--mint)" }}><div className="h-full rounded-full" style={{ width: `${v}%`, background: dry ? "var(--warn)" : "var(--green)" }} /></div></div>); })}</div>
        </div>
