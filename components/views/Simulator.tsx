        ...
        <div className="grid md:grid-cols-3 gap-6 items-center">
          <div className="md:col-span-2">
            ...
            <div className="flex items-end gap-6 mt-4">
              <div><p className="text-[11px] uppercase tracking-wider" style={{ color: "rgba(255,255,255,.6)" }}>{t("Projected")}</p><AnimatedNumber value={calc.net} format={(n) => formatMoney(n, currency)} className="mono text-5xl font-bold block" style={{ color: "var(--lime)" }} /></div>
              <div className="pb-1"><p className="text-[11px] uppercase tracking-wider" style={{ color: "rgba(255,255,255,.6)" }}>{t("vs. doing nothing")}</p><p className="mono text-xl font-bold" style={{ color: calc.delta >= 0 ? "var(--lime)" : "#ff9f7a" }}>{calc.delta >= 0 ? "+" : "-"}{formatMoney(Math.abs(calc.delta), currency)}</p></div>
            </div>
          </div>
          <div className="rounded-2xl p-5" style={{ background: "rgba(255,255,255,.07)", border: "1px solid rgba(255,255,255,.12)" }}>
            {[["Base", formatMoney(BASE_MARGIN, currency), "#fff"], ["Recovered", "+" + formatMoney(calc.rec, currency), "var(--lime)"], ["Action cost", "-" + formatMoney(calc.cost, currency), "#ff9f7a"], ["Residual loss", "-" + formatMoney(calc.res, currency), "#ff9f7a"]].map(([l, v, c]) => (
              <div key={l} className="flex justify-between text-xs mb-2"><span style={{ color: "rgba(255,255,255,.7)" }}>{t(l)}</span><span className="mono font-semibold" style={{ color: c }}>{v}</span></div>
            ))}
            <div className="flex justify-between text-sm mt-3 pt-3 font-bold" style={{ borderTop: "1px solid rgba(255,255,255,.14)" }}><span>{t("Net")}</span><span className="mono" style={{ color: "var(--lime)" }}>{formatMoney(calc.net, currency)}</span></div>
          </div>
        </div>
      </div>
      <div className="grid lg:grid-cols-5 gap-5">
        <div className="lg:col-span-3 card p-6">
          <h4 className="text-[15px] font-bold mb-1">{t("Decision levers")}</h4>
          ...
          <div>
            {CONFLICTS.map((c, i) => (
              <div key={c.id} className={`flex items-center gap-3 py-4 ${i > 0 ? "border-t border-line" : ""}`}>
                ...
                <div className="flex-1"><p className="text-sm font-bold leading-tight">{t(c.actionLabel)}</p><p className="text-xs mt-0.5 text-muted">{c.parcel} · {t("recovers")} <span className="mono font-semibold text-green">{formatMoney(c.loss, currency)}</span> · {t("costs")} <span className="mono">{formatMoney(c.actionCost, currency)}</span></p></div>
          ...
          <div className="mt-5 pt-5 border-t border-line">
            <div className="flex items-center justify-between mb-3"><div><p className="text-sm font-bold">{t("Accepted delay days")}</p><p className="text-xs text-muted">{t("How far you tolerate shifting harvest")}</p></div><span className="mono text-lg font-bold px-3 py-1 rounded-lg" style={{ background: "var(--mint)" }}>{delay} d</span></div>
