    ...
    <div className="fade-in grid lg:grid-cols-3 gap-5">
      <div className="lg:col-span-1">
        <div className="rounded-2xl p-5 mb-4" style={{ background: "var(--ink)", color: "#fff" }}>
          <p className="text-[11px] uppercase tracking-wider" style={{ color: "rgba(255,255,255,.6)" }}>{tr("Margin saved today")}</p>
        ...
        <div>
          {FEED.map((f, i) => (
            <div key={i} className={`flex gap-3 py-3 ${i > 0 ? "border-t border-line" : ""}`}>
              ...
              <div className="flex-1 min-w-0"><span className="text-[10px] mono text-muted">{f.t}</span><p className="text-sm font-semibold leading-tight mt-0.5">{tr(f.title)}</p><p className="text-xs mt-0.5 text-muted">{tr(f.desc)}</p></div>
              <span className="mono text-xs font-bold self-center shrink-0" style={{ color: f.warn ? "var(--warn)" : "var(--green)" }}>{tr(f.val)}</span>
            </div>
          ))}
        </div>
      </div>
