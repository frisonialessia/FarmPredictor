  ...
  return (
    <div className="fade-in">
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-5">
        {FINK.map((k) => (
          <div key={k.l} className="card p-5"><p className="text-xs text-muted">{k.l}</p><p className={`mono text-2xl font-bold mt-2 ${k.hl ? "text-green" : ""}`}>{formatMoney(k.base, currency)}</p><p className="text-xs mt-1 text-muted">{k.s}</p></div>
        ))}
        <div className="card p-5"><p className="text-xs text-muted">Committed</p><p className="mono text-2xl font-bold mt-2">68%</p><p className="text-xs mt-1 text-muted">in sales contracts</p></div>
      </div>
      <div className="grid lg:grid-cols-5 gap-5 mb-5">
        <div className="lg:col-span-3 card p-6">
          <h4 className="text-[15px] font-bold mb-1">Margin by parcel</h4><p className="text-xs mb-4 text-muted">Estimated net profit per acre</p>
        ...
        <div className="lg:col-span-2 card p-6">
          <h4 className="text-[15px] font-bold mb-1">Projected cash flow</h4><p className="text-xs mb-4 text-muted">Next 6 weeks</p>
          ...
          <div className="flex items-center justify-between mt-4 pt-4 border-t border-line text-sm"><span className="text-muted">Committed in contracts</span><span className="mono font-bold">68%</span></div>
        </div>
      </div>
      <div className="card p-6">
        <h4 className="text-[15px] font-bold mb-1">Sales contracts</h4><p className="text-xs mb-4 text-muted">Committed volume vs. open</p>
        <div>
          {CONTRACTS.map((c, i) => (
            <div key={i} className={`flex items-center gap-4 py-3 ${i > 0 ? "border-t border-line" : ""}`}>
              <div className="flex-1"><p className="text-sm font-semibold">{c[0]}</p><p className="text-xs text-muted">{c[1]} · {c[2]}</p></div>
