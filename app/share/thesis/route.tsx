...
const LIME = "#85df42", GREEN = "#52c871", WARN_SOFT = "#ff9f7a";
...
export async function GET() {
  return new ImageResponse(
    (
      <div style={{ width: "100%", height: "100%", display: "flex", flexDirection: "column", justifyContent: "space-between", padding: 64, background: "linear-gradient(135deg,#0B0F0C 0%,#143b1f 58%,#1d5c2e 100%)", color: "#fff", fontFamily: "sans-serif" }}>
        {/* brand + headline */}
        <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", fontSize: 52, fontWeight: 800, letterSpacing: -1, lineHeight: 1.06 }}>
          ...
          <span style={{ fontSize: 21, color: "rgba(255,255,255,.72)", maxWidth: 720 }}>One engine: the Planner and the Simulator share the same plan. A conflict you create shows up here automatically — in dollars.</span>
        </div>

        {/* simulator panel */}
        <div style={{ display: "flex", background: "rgba(255,255,255,.06)", border: "1px solid rgba(255,255,255,.12)", borderRadius: 20, padding: 28, gap: 32 }}>
          <div style={{ display: "flex", flexDirection: "column", flex: 1, justifyContent: "center" }}>
            ...
            <span style={{ fontSize: 13, textTransform: "uppercase", letterSpacing: 1, color: "rgba(255,255,255,.55)" }}>Weekly net margin</span>
            <span style={{ fontSize: 72, fontWeight: 800, color: LIME, lineHeight: 1.1 }}>$26,296</span>
            <span style={{ fontSize: 16, color: WARN_SOFT, marginTop: 4 }}>−$7,904 below the executable ceiling</span>
          </div>
          <div style={{ display: "flex", flexDirection: "column", width: 320, background: "rgba(255,255,255,.05)", border: "1px solid rgba(255,255,255,.10)", borderRadius: 14, padding: 18, justifyContent: "center" }}>
            {ROWS.map(([l, v, c]) => (
            ...
            <div style={{ display: "flex", justifyContent: "space-between", fontSize: 17, fontWeight: 800, marginTop: 6, paddingTop: 12, borderTop: "1px solid rgba(255,255,255,.16)" }}>
              <span>Net</span><span style={{ color: LIME }}>$26,296</span>
            </div>
          </div>
        </div>
      </div>
