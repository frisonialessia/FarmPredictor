...
export async function GET() {
  ...
  return new ImageResponse(
    (
      <div style={{ width: "100%", height: "100%", display: "flex", alignItems: "center", justifyContent: "center", background: BG, padding: 34, fontFamily: "Jakarta" }}>
        <div style={{ width: 1132, display: "flex", flexDirection: "column", background: "#fff", borderRadius: 18, overflow: "hidden", boxShadow: "0 24px 60px rgba(11,15,12,.18)", border: `1px solid ${LINE}` }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "13px 18px", background: BG, borderBottom: `1px solid ${LINE}` }}>
          ...
          <div style={{ display: "flex" }}>
            <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 10, padding: "16px 12px", borderRight: `1px solid ${LINE}`, width: 70 }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", flex: 1, padding: 22, gap: 14 }}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                <div style={{ display: "flex", flexDirection: "column" }}>
                  <span style={{ fontSize: 19, fontWeight: 800, letterSpacing: -0.3 }}>Overview</span>
                  <span style={{ fontSize: 13, fontWeight: 400, color: MUTED }}>Tuesday, June 9 · 08:14</span>
                </div>
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 6, padding: "5px 12px", border: `1px solid ${LINE}`, borderRadius: 99, fontSize: 12, fontWeight: 600, color: MUTED }}><div style={{ width: 7, height: 7, borderRadius: 99, background: GREEN }} />Demo data</div>
              ...
              <div style={{ display: "flex", gap: 14 }}>
                {KPIS.map(([label, value, hl]) => (
                  <Card key={label} style={{ flex: 1, padding: "15px 18px" }}>
                    <span style={{ fontSize: 11, fontWeight: 700, letterSpacing: 1, textTransform: "uppercase", color: MUTED }}>{label}</span>
                    <span style={{ fontSize: 28, fontWeight: 700, marginTop: 6, color: hl ? GREEN_DEEP : INK, fontFamily: "Plex" }}>{value}</span>
                  </Card>
                ))}
              </div>
              ...
              <div style={{ display: "flex", gap: 14 }}>
                <Card style={{ flex: 2, padding: 20 }}>
                  ...
                  <span style={{ fontSize: 12, fontWeight: 400, color: MUTED, marginTop: 2, marginBottom: 4 }}>Sorted by margin impact</span>
                  {DECISIONS.map(([parcel, title, loss], i) => (
                    <div key={i} style={{ display: "flex", alignItems: "center", gap: 12, padding: "10px 0", borderTop: i > 0 ? `1px solid ${LINE}` : "none" }}>
                      ...
                      <div style={{ display: "flex", flexDirection: "column", flex: 1 }}>
                        ...
                        <span style={{ fontSize: 13, fontWeight: 400, color: INK }}>{title}</span>
                      </div>
                      <span style={{ fontSize: 15, fontWeight: 700, color: WARN, fontFamily: "Plex" }}>{loss}</span>
                    </div>
                  ))}
                </Card>
                ...
                <Card style={{ flex: 1, padding: 20 }}>
                  ...
                  <span style={{ fontSize: 12, fontWeight: 400, color: MUTED, marginTop: 2 }}>Recovered, by month</span>
                  <div style={{ display: "flex", alignItems: "flex-end", gap: 8, height: 104, marginTop: 14 }}>
                    {SEASON.map((v, i) => (
                      <div key={i} style={{ display: "flex", flexDirection: "column", alignItems: "center", flex: 1, height: "100%", justifyContent: "flex-end" }}>
                        <div style={{ display: "flex", width: "100%", height: `${(v / 42) * 100}%`, background: i === 5 ? GREEN : MINT, borderRadius: "6px 6px 0 0" }} />
                        <span style={{ fontSize: 10, fontWeight: 400, color: MUTED, marginTop: 4 }}>{MONTHS[i]}</span>
                      </div>
                    ))}
                  </div>
                </Card>
